...
pie_gender_data <- fread("data/pie_gender_data.csv", header = TRUE)
sun_df <- fread("data/sun_df.csv", header = TRUE)
Class_Counts <- fread("data/class_counts.csv", header = TRUE)
...
MediumsCorpus <- tm_map(MediumsCorpus, removeWords, c("a", "an", "the", "on", "from", "with", "and"))
